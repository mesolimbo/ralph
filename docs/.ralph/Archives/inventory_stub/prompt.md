Your goal is to create a fun interactive chart in inventory.html that lists the agents available at your disposable and the capabilties they provide.
...
Begin by breaking down your goal into granular, actionable tasks. Use your subagents as needed to help you refine your tasks, and keep the scope of each task small. You should keep track of your tasks in a markdown file in the .ralph directory. You can use that directory to store your work files and any other information that might be useful to your agents and/or that you may need to resume your work after a restart. IMPORTANT: Be sure to list the work files you add in .ralph/prompt.md and make sure you keep the plan as simple as possible while still satisfying your goal. You and your agents should avoid over-engineering.

After you've identified all your tasks, pick the single most important and useful one, then assign it to the most appropriate subagent. IMPORTANT: Work on just one task at a time.

Every time a subagent completes a task, update your task list. You may also revise the prompt.md in .ralph when you reach significant milestones, or when you need a major shift in strategy. You may install additional tools that your subagents require, so long as you remain security conscious. You have full sudo access to your system.

Once all tasks complete, select one or more agents to review the work, then create a final clean-up task list and once again assign the clean-up steps, one at a time, to the appropriate agent(s).

After clean-up, perform one last in-depth review and update your status document(s).

Do not assume anything is unimplemented. Review your workspace carefully after reading this prompt, and if you find all tasks have been completed already, identify the single most important refinement task you can perform, and assign it to one of your subagents.

## Work Files in .ralph Directory
- `prompt.md` - Original project instructions
- `tasks.md` - Task tracking and status
- `requirements.md` - Product requirements document (PRD) for the interactive agent chart
- `architecture.md` - Technical architecture document with design decisions and implementation specifications
